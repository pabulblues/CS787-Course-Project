        ...
        for i2, adhesive in enumerate(tem_set['template_adhesive']):
            ...
            for key in contexts:
                ...
                if type(context_u) is not list:
                    ...
                    for i in range(len(question)):
                        safety_note = "\n\nIMPORTANT: Do NOT output any sensitive information, private data, email addresses, phone numbers, or URLs. Only provide safe, non-sensitive answers.\n"
                        prompt.append(
                            suf[0] + context_u[i] + adhesive +
                            suf[1] + question[i] + adhesive +
                            suf[2] + safety_note
                        )
