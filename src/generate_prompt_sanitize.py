...
def sanitize(text):
    """
    Sanitizes text for common PII and returns:
        (cleaned_text, number_of_substitutions)
    """
    if not text:
        return text, 0
    ...
    patterns = [
        ...
        (r'\b(\+?\d{1,2}[-.\s]?)?(\(?\d{3}\)?[-.\s]?){1,2}\d{3,4}\b', '<PHONE>'),  # phone
...
def get_contexts(data_name_list: List[List[str]],
                                    ...
                                    for i8, sum_ in enumerate(summarize):
                                        ...
                                        for i, que in enumerate(ques):
                                            ...
                                            for con in ori_context:
                                                ...
                                                clean_text,num= sanitize(text)
                                                num_subs+=num
                                                t_cons.append(clean_text)
    ...
    print("The number of replacements done are:")
    print(num_subs)
